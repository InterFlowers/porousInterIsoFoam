...
void Foam::isoAdvection::timeIntegratedFlux()
{
    ...
    forAll(interfaceLabels, i)
    {
        ...
        if (mag(surf_->normal()[celli]) != 0)
        {
            ...
            // Get the speed of the isoface by interpolating velocity, 
            // dotting it with isoface unit normal and deviding 
            // it by the porosity value of the particular cell
            const scalar Un0 =(1/porosityIn_[celli])*UInterp.interpolate(x0, celli) & n0;  
            DebugInfo
